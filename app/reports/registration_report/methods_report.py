...
def classify_by_rules(item):
    """
    Fallback-логика, если субагент не найден
    в Google Sheets Mapping.
...
def build_country_report(
    ...
    - Bolivia:
        QR code payment -> подстроки с префиксом "QR"
        Pago Facil -> подстроки с префиксом "Pago Facil"
        QR Rapido -> подстроки с префиксом "QR Rapido"

    - Mexico:
        Spei -> подстроки с префиксом "Spei"
    """
    ...
    for group_name, group_items in groups.items():
        ...
        if is_detail_group:
            ...
            for item in group_items:
                ...
                # ------------------------------------------------
                # Префикс для названия субагента
                # ------------------------------------------------
                ...
                if country == "Мексика" and group_name == "Spei":

                    prefix = "Spei"

                elif country == "Боливия" and group_name == "Pago Facil":

                    prefix = "Pago Facil"

                elif country == "Боливия" and group_name == "QR code payment":

                    prefix = "QR"

                elif country == "Боливия" and group_name == "QR Rapido":
    ...
    # Если были маленькие группы,
    # объединяем их в "Остальные методы"
